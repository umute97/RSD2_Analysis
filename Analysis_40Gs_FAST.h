...
Double_t pippo[MAX_CAMP];
Double_t pippoS[MAX_CAMP];
...
std::ostringstream leafl(std::ostringstream::out);
  ...
  // Program variables
std::ostringstream datafile[NCHRO];
//(std::ostringstream::out);
std::ifstream data[NCHRO];
std::ifstream dat;
  ...
  TValues[NArray] = -100;
      ...
      if(((amp[j]-bck)/max-threshold)<0 && threshold>0)
	{
	  if (Interpolation)
	    {
	      Nth = j;
	      C2 = (amp[j+1]-bck)/max; // Above Vth; too late 35
	      C1 = (amp[j]-bck)/max;  // Below Vth; too early 15
	      // (30 -15)/(35-15)
	      if ( C2>C1 ) TCrossing =  timeS[j]+DT*(threshold-C1)/(C2-C1); //time[j] is too early, so I need to add a fraction of time bin	  
	    }
	  else  TCrossing =  timeS[j];
	  // TCrossing =  timeS[j];
	  NTValues[NArray] = Nth;
