...
void Amplitudes(Int_t camp, Double_t amp[], Double_t timeS[], Double_t bck, Double_t t_bck, Double_t t_pul, Double_t *pamp, Double_t *tamp, Double_t *chi)
{
  ...
  *tamp = tmax;
  ...
  // std::cout << j_start << " " <<j_max << " Number of points in the fit= " << n_j << " Maximum w/o fit = " << max <<std::endl;

  if (n_j < 4)
    return; // not enough points in the fit
  ...
  //  g.Fit("gausfit","QN","",timeS[j_start]-1,timeS[j_start+n_j-1]+1);
  if (fabs(f1->GetParameter(0)) - max < 40)
  {
    *pamp = f1->GetParameter(0);
    *tamp = f1->GetParameter(1);
  }
  //  else
  //  std::cout << "Fitting problem for the amplitude" << std::endl;
  ...
  *thrtime = 0;
  ...
  for (int j = NMax; j > 1; j--)
  {
    ...
    if (((amp[j] - bck) / max - threshold) < 0 && threshold > 0)
    {
      ...
      if ((C2 - C1) != 0)
        TCrossing = timeS[j] + (timeS[j + 1] - timeS[j]) * (threshold - C1) / (C2 - C1);
      break;
    }
  }
  //  std::cout << "th1 = " << *thrtime << std::endl;
  ...
  *IMaxSh = 7. / TIMEUNIT; // TimeUnit in [ns]
  ...
  for (int i = 0; i < camp; i++)
  {
    ...
    for (int ll = 0; ll < *IMaxSh - i; ll++)
    {
      ...
      tt = ll * TIMEUNIT;
      tau = 0;
      ShaperOut_Q[i + ll] += Qdif_Shaper * (-1. * tac * tr) * (((-1. * exp(-(tt - tau) / tac) * tac) / (pow((tac - til), 2.) * (tac - tr))) + ((exp(-(tt - tau) / tr) * tr) / (pow((til - tr), 2.) * (tac - tr))) + ((exp(-(tt - tau) / til) * (pow(til, 3.) + (tt - tau) * (tac - til) * (til - tr) - tac * til * tr)) / (pow((tac - til), 2.) * til * pow((til - tr), 2.))));

      if (fabs(ShaperOut_Q[i + ll]) > fabs(sh_max))
        sh_max = ShaperOut_Q[i + ll];
    }
  }
...
void BBA(Double_t TIMEUNIT, Double_t Cdet, Double_t BBImp, Double_t BBGain, double_t BBABW, Int_t camp, Double_t Itot[], Double_t BBGraph[], Int_t *IMaxSh)
{
  ...
  *IMaxSh = 2. / TIMEUNIT;                         // Number of time steps to be computed
  ...
  for (int i = 0; i < camp; i++)
  {
    ...
    if (i == 0)
      Qi = Itot[0] * 1e-6 * (TIMEUNIT);
    else
      Qi = (Itot[i] - Itot[i - 1]) * 1e-6 * (TIMEUNIT); // This is the charge on the Cap added in step i
  ...
  *IMaxSh = 12. / TimeUnit; // TimeUnit in [ns]
  ...
  for (int i = 1; i < IMax; i++)
  {
    ...
    for (int ll = 0; ll < *IMaxSh - i; ll++) // valid only up to IMaxSh
    {
      ...
      if (fabs(CSAout[i + ll]) > fabs(CSAMax))
        CSAMax = CSAout[i + ll];
    }
    ...
    if (((amp[j] - bck) / max - threshold) < 0 && threshold > 0)
    {
      if (Interpolation)
      {
        *Nth = j;
        ...
        if ((C2 - C1) != 0)
          TCrossing = timeS[j] + DT * (threshold - C1) / (C2 - C1); // time[j] is too late, so I need to subtract a fraction of time bin
      }
      else
        TCrossing = timeS[j];
    ...
    if ((amp[j] - bck - threshold) < 0 && (amp[j - 1] - bck - threshold) > 0 && threshold > 0)
    {
      if (Interpolation)
      {
        *Nth = j;
        ...
        if ((C2 - C1) != 0)
          TCrossing = timeS[j] - DT * (threshold - C1) / (C2 - C1);
      }
      else
        TCrossing = timeS[j];
    ...
    if (((amp[j] - bck) / max - threshold) < 0 && threshold > 0)
    {
      if (Interpolation)
      {
        *Nth = j;
        ...
        if ((C2 - C1) != 0)
          TCrossing = timeS[j] - DT * (threshold - C1) / (C2 - C1); // timeS[j] is too late, I need to subtract  a bit of DT
    ...
    if ((amp[j] - bck - threshold) < 0 && (amp[j + 1] - bck - threshold) > 0 && threshold > 0)
    {
      if (Interpolation)
      {
        *Nth = j;
        ...
        if ((C2 - C1) != 0)
          TCrossing = timeS[j] + DT * (threshold - C1) / (C2 - C1); // time[j] is too early, so I need to add a fraction of time bin
      }
      else
        TCrossing = timeS[j];
...
void Levtime_N_array(int NTValues[], double TValues[], Int_t NArray, Double_t Threshold[], Double_t amp[], Double_t DT, Int_t NMax, Double_t bck, bool Interpolation)
{
  ...
  for (int j = NMax; j > 0; j--)
  {
    ...
    if ((amp[j] - bck - threshold) < 0 && (amp[j + 1] - bck - threshold) > 0 && threshold > 0)
    {
      ...
      NArray--;
      while (amp[j] - bck < Threshold[NArray])
        j++;
    }
  }
