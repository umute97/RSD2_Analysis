...
int main()
{
	...
	// TODO: These seem to be the pad positions
	...
	while (1)
	{
		...
		cout << endl
			 << " File name:" << endl
			 << title << " averaging " << nme << " points " << endl;
		...
		if (nme < 0)
		{
			...
			nmedia[2] = 3;
			cout << endl
				 << "Channel 0  with Freq. Cut = " << FreqCut[0] * 1e-6 << " MHz" << endl;
		...
		metaTree->SetBranchAddress("pos", &pos, &b_pos);
		metaTree->SetBranchAddress("n", &nSize, &b_size);
		...
		while (running) // running is set on 1 as long as all the channels have data for a trigger event
		{
			...
			if (eventperpoint > Max_eventperpoint && Max_eventperpoint > -1)
				continue;
			...
			for (i = 0; i < nchro; i++)
			{
				...
				if (i == 0 && ntrig == 0)
				{
					cout << "Number of samples in the acquisition = " << nSize << endl;
				...
				if (np < np_Max - 10 && event < 20)
					cout << "Warning: file reading ended too soon: read point np = " << np << " instead of np_max = " << np_Max << endl;
				...
				if (ntrig == 2 && i == 0)
					cout << "Number of points: " << np << " with a time step of " << DT << " [ns] " << endl;

				if (ntrig < 20)
					cout << "Event: " << event << " channel " << i << " has maximum value of " << AMax[i] << " [mV] at  " << TMax[i] << " [ns] "
						 << " on sample " << NMax[i] << " polarity = " << polarity[i] << " at position x-y " << pos[0][0] << ", " << pos[0][1] << endl;
				...
				if (NArr > 0)
					Levtime_N_array(&NTValuesL[0], &TValuesL[0], NArr, &ThresholdL[0], &m_amprec[i][0], DT, NMax[i], bck[i], Interpolation);
				if (TrailNArr > 0)
					TrailLevtime_N_array(&TrailTValuesL[0], TrailNArr, &TrailThresholdL[0], samples[i], &m_amprec[i][0], DT, NMax[i], bck[i], Interpolation);

				// Calculate Time of Constant Fraction values

				if (ampl[i] > 10)
					ConstFractime_N_array(&NTValuesF[0], &TValuesF[0], NArrayF, &ThresholdF[0], &m_amprec[i][0], DT, bck[i], ampl[i], NMax[i], Interpolation);
				if (ampl[i] > 10)
					TrailConstFractime_N_array(&TrailNTValuesF[0], &TrailTValuesF[0], TrailNArrayF, &TrailThresholdF[0], samples[i], &m_amprec[i][0], DT, bck[i], ampl[i], NMax[i], Interpolation);
				...
				//	      cout << "check 3 " << i << endl;
				//
				if (((cfd10[i] / DT) < samples[i] && (cfd20[i] / DT) < samples[i] && (cfd30[i] / DT) < samples[i] && (cfd70[i] / DT) < samples[i] && (cfd80[i] / DT) < samples[i]) &&
					((cfd10[i] / DT) > 0 && (cfd20[i] / DT) > 0 && (cfd30[i] / DT) > 0 && (cfd70[i] / DT) > 0 && (cfd80[i] / DT) > 0))
				{
				...
				if (doFit)
				{
					...
					if (rise_lin1[i] != 0)
						t_zero[i] = (-rise_lin0[i]) / rise_lin1[i];
